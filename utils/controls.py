...
def playlists_global(uri):
    ...
    action = input("Clify/Playlist/Action > ")
    if action == "back":
        return "back"
    elif action == "play":
...
def spotify_search_global(args):
    def play_search(check_quick):
        try:
            track_number_int = 0 if check_quick else int(input("Clify/Search/Play > "))
    ...
    def add_queue_search(check_quick):
        try:
            track_number = 0 if check_quick else int(input("Clify/Search/Add To Queue > "))
